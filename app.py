...
if data_to_process:
    ...
    if all_records:
        ...
        # 탭 구성
        # ---------------------------------------------------------
        tab1, tab2, tab3 = st.tabs(["📈 생산 추이 및 요약", "📝 OPEN ISSUE 정밀 분석", "📅 상세 데이터 조회 (선택 조건)"])
        ...
        with tab1:
            ...
            if not plot_df.empty:
                colors = ['#FF4B4B' if safe_float(row[y_val]) < safe_float(row['목표효율']) else '#1F77B4' for _, row in plot_df.iterrows()]
